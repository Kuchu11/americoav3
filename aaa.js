function validarterestaurante(order) {
...
function identificacaoCliente(order) {
...
function verificacaodeendereco(order) {
...
function desconto(subtotal, couponCode) {
  let discount = 0;
  if (order.couponCode) {
...
function calculardelivery(distance, subtotal) {
  let deliveryFee = 0;
...
function calculardeservicofree(subtotal) {
...
function sacoladepedidos(items) {
  for (let item of order.items) {
    console.log(`Reduzindo estoque do item ${item.id} em ${item.quantity} unidades`);
  }

  function registrodepedido(order, subtotal, desconto, deliveryFee, serviceFee, total) {
    return {
      id: `IFOOD-${Date.now()}`,
